...
class CronController extends Controller
{
    ...
    public function processBulkData()
    {
        ...
        $custom_log->debug("\n\n\n--------------------Start The processBulkData Here---------------------------Temporary Exit---------");
        die('Temporary Exit');
        ...
        if(!empty($fileData))
        {
            ...
            $custom_log->debug(__LINE__." : VehicleList : ". json_encode($vehicleArr));
            if (!empty($vehicleArr)) {
                    ...
                    if($isLogExist === false)
                    {
                        ...
                        if ($isValidVehicleNumber === false) {
                            ...
                            if (empty($response)) 
                            {
                                ...
                                $custom_log->debug(__LINE__." : headers : ". json_encode($headers));
                            ...
                            else 
                            {
                                ...
                                {
                                    $response   = json_encode(['vehicleNo' => $vehicleNo, 'status_code' => $statusCode, 'message' => 'Error: data is not valid']);
                                    $custom_log->debug(__LINE__." : ".$response);
                                }
                        ...
                        $api_log->response_message  = $message;
        ...
        // if($fileData->retry_attempts == 2)
        // {
        //     // $resetFlag = $this->resetBulkProcessFlag();
        //    // $custom_log->debug(__LINE__." : resetFlag - ". $resetFlag);
        // }
