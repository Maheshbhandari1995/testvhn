        ...
        {	
            if (isset($sessionData) && $sessionData['userRole'] == 'admin') {
                ...
                {       
                    ...
                    ->count();
                    /////////////////////
                    // $yesterdaysuccessCount = DB::table('api_log')
                    // ->where('api_log.client_id', '5')
                    // ->whereDate('api_log.created_at', '2023-12-04')
                    ...
                    // ->count();
                    /////////////////////////////////
                    // $yesterdaysuccessCount = Rcdetails::whereIn('response_status_code', [200, 401, 1])
                    // ->where('api_log.client_id', $Client_id)
                    // ->whereDate('api_log.created_at', $yesterday)
                    // ->distinct(['input'])
                    // ->count();
                    ...
                    ///2   ////all portal yeterday hits hits

                    //$yesterdayutilizedCount = Rcdetails::where('api_log.client_id', $Client_id)
                    // ->whereDate('api_log.created_at', $yesterday)
                    // ->distinct(['input'])
                    // ->count();
                    ///////////////////////////
                    ...
                    // {3}
                    ///////////////////////
                    // $yesterdayFailCount = DB::table('api_log')
                    //     ->whereNotIn('response_status_code', [200, 401, 1])
                    //     ->where('api_log.client_id', $Client_id)
                    //     ->whereDate('api_log.created_at', $yesterday)
                    //     ->whereNotIn('input', function ($query) {
                    //         $query->select('input')
                    //             ->from('api_log')
                    //             ->where('api_log.client_id', $Client_id)
                    //             ->whereDate('api_log.created_at', $yesterday)
                    ...
                    $monthportalhits = DB::table('api_log')
                    ->where('api_log.client_id', $Client_id)
                    ->whereRaw("DATE(api_log.created_at) BETWEEN ? AND ?", [$firstDayOfMonth, $yesterday])
                    ->count();

                    // $monthportalhits = Rcdetails::where('users.client_id', $Client_id)
                    // ->join('users', 'users.id', '=', 'api_log.user_id')
                    // ->whereRaw("DATE(api_log.created_at) BETWEEN ? AND ?", [$firstDayOfMonth, $yesterday])
                    // ->count();
                   ...
                   // ->distinct(['input'])
                    ...
                    ->count();
                     // {6}
                     ////////////////////////////
                    //  $yesterdayFailCount = DB::table('external_source_data')
                    //  ->whereNotIn('status_code', [200, 401, 1])
                    //  ->whereDate('created_at', $yesterday)
