...
class RcBulkUploadController extends Controller
{
    ...
    public function authbridgeRCBulkData(Request $request)
    {
       ...
       // echo "<pre>"; print_r($sessionData);die;
        $request->validate([
            'rcdata' => 'required|file|mimes:csv,txt|max:5048', // Adjust the max file size as per your requirements
        ]);
        ...
        if ($request->hasFile('rcdata')) {
            ...
            if($fileExists)
            {
                ...
                if($vehicleNumbers[0] != 'vehicle_no')
                {                    
                    return response()->json(['status'=> 'failed', 'msg' => 'Invalid File Format, Please keep header name as {vehicle_no}.']);
                }
                ...
                {
                    return response()->json(['status'=> 'failed', 'msg' => 'You can not contain more then 10000 vehicles no in one file.']);

                }
                else
                {
					//echo "<pre>"; print_r($vehicleNumbers);die;
