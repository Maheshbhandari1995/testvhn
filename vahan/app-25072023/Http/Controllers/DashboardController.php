        ...
        if ($request->ajax()) {
            ...
            if (isset($sessionData) && $sessionData['userRole'] == 'user') {
                ...
                $data = DB::table('api_log')
                    ...
                    ->select('api_log.api_name', 'api_log.api_url', 'api_log.response_status_code', 'api_log.created_at')
                    ->get();
                ...
                $Client_id = $sessionData['Client_id'];  
                    ...
                    ->select('users.username', 'api_log.status', DB::raw('COUNT(*) as count'))
                    ->where('api_log.client_id', '=', $Client_id)
                    ->groupBy('users.username', 'api_log.status')
                    ->get();


                    $totalCounts = Rcdetails::join('users', 'users.id', '=', 'api_log.user_id')
                        ->where('api_log.client_id', '=', $Client_id)
                        ->groupBy('api_log.client_id')
                        ->selectRaw('COUNT(*) as count')
                        ->get();

                    $successCounts = Rcdetails::join('users', 'users.id', '=', 'api_log.user_id')
                    ...
                    $failCounts = Rcdetails::join('users', 'users.id', '=', 'api_log.user_id')
                        ->where('response_status_code', '!=', 200)
                        ->where('api_log.client_id', '=', $Client_id)
                        ->groupBy('api_log.client_id')
                        ->selectRaw('COUNT(*) as count')
                    ...
                    return DataTables::of($data)
                    ->addColumn('totalCounts', function() use ($totalCounts) {
                         return $totalCounts[0]->count;
                    })
                    ->addColumn('successCounts', function() use ($successCounts) {
                        return $successCounts[0]->count;
                    })
                    ->addColumn('failcounts', function() use ($failCounts) {
                        return $failCounts[0]->count;
                    })
                    ->make(true);
