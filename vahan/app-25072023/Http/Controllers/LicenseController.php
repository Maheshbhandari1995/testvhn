...
class LicenseController extends Controller
{
    ...
    {   
       ...
       // $sevenDaysAgo = Carbon::now()->subDays(7)->toDateString();
        $result = DB::select("SELECT id, response FROM `history_license` WHERE license_no = '$dl_number' AND dob='$dob' AND vendor = '$vendorname' AND `status` IN (0,1) AND created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY) ORDER BY id DESC LIMIT 1");
        if (!empty($result)) {
