...
class BulkCronController extends Controller
{
        ...
        if(!empty($fileData))
        {
            ...
            if (!empty($vehicleArr)) {
                ...
                foreach ($vehicleNumbers as $key => $input) {
                    ...
                    $custom_log->debug(__LINE__." :  ------ Process Start for Vehicle No  ---- ".$vehicleNo."\n\n\n"); 
                    ...
                    if($isLogExist == false)
                    {
                        ...
                        {
                            $response = json_encode(['vehicleNo' => $vehicleNo, 'status_code' => '101', 'message' => 'You do not have enough credit to perform this action']);                           
                        }
                        ...
                        if ($isValidVehicleNumber === false) {
                            ...
                            if(isset($response) && empty($response)) 
                            {
                                ...
                                if((isset($responseArr['status']) && $responseArr['status'] != 'success'))
                                {
                                  //  echo "<pre> secondaryVendor -- : "; print_r($secondaryVendor);//die;
                                    ...
                                    if(isset($secondaryVendor) && !empty($secondaryVendor))
                                    {
                                        ...
                                        // $response_message= $responseArr['status'];
                                        if((isset($responseArr['status']) && $responseArr['status'] == 'success'))
                                        {
                                ...
                                $statusCode     = 200;
                                $url            = '';
                                $requestData    = json_encode(array($vehicleNo));
    ...
    protected function downlaodDumpRCList($id)
    {
        ...
        $results = DB::table('cron_bulk_dump')
            ...
            ->select('api_log.response', 'cron_bulk_dump.input', 'api_log.response_status_code', 'vender')
            ->whereIn('api_log.status', [0,1])
            ->where('bulk_id', $id)
            ->orderBy('api_log.id', 'asc')
            ->get()
            ...
            if(!empty($status_code) && $status_code != '200')
            {
                ...
                if(isset($response['result'])){
                    ...
                    $csvData = [
                        ...
                        isset($msg['class']) ? $msg['class']  : null, // Vehicle Class
                        isset($msg['type']) ? $msg['type'] : null, // type
                        isset($msg['chassis']) ? $msg['chassis'] : null, // Chassis Number
                        isset($msg['chassisengine']) ? $msg['chassisengine']  : null, // Engine Number
                        ...
                        isset($msg['Engine Capacity']) ? $msg['Engine Capacity'] : null, // Engine Capacity -----
